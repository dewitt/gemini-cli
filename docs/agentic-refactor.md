...
1.  **Configuration Check:** Inspect `config.experimental.useExternalRuntime`
    (or similar flag).
2.  **Legacy Path:** If false, instantiate and return the existing internal loop
    implementation (wrapped to implement `Agent`).
3.  **External Path:** If true, dynamically import and instantiate an Adapter
...
2.  **FrameworkXModelAdapter**:
    ...
    - **Responsibilities:**
      ...
      - Convert Gemini response chunks into FrameworkX's expected response
        format.
      - Handle error translation (ensure errors are thrown in a format
        FrameworkX understands).

3.  **FrameworkXToolAdapter**:
...
2.  Factory checks config. If enabled, creates `FrameworkXAgentAdapter`.
3.  `SubagentTool` calls `agent.runEphemeral()`.
4.  `FrameworkXAgentAdapter` starts the external runner.
5.  External runner decides to call a tool (e.g., `ls`).
6.  It calls `FrameworkXToolAdapter.execute('ls')`.
7.  Adapter runs internal `ls` tool.
8.  External runner receives output, generates thought.
9.  `FrameworkXAgentAdapter` captures thought, yields `{ type: 'thought' }`.
10. CLI UI renders the thought stream.
...
2.  Factory returns `FrameworkXAgentAdapter` configured with
    `FrameworkXModelAdapter`.
3.  User enters text. `GeminiClient` calls
    `agent.runAsync(text, { sessionId })`.
4.  `FrameworkXAgentAdapter` resumes the session and streams events.
    ...
    - **Tool Calls:**
      ...
      - _Option B (UI-Driven/HITL):_ FrameworkX yields `{ type: 'tool_call' }`
        ...
        user approval, `GeminiClient` invokes the tool and resumes the agent
        with the result.
5.  `GeminiClient` handles the event stream, updating the UI in real-time.

## 6. Implementation Checklist

1.  **Define Interfaces:** Create `Agent` and `Model` interfaces in
    `packages/core`.
2.  **Refactor Legacy:** Refactor `LocalAgentExecutor` to implement `Agent` and
    move its logic into a `LegacyLoop` class.
3.  **Implement Factory:** Update `LocalAgentExecutor.create` to switch
    implementations based on config.
4.  **Create Adapters:** Implement the three adapter classes for the target
    external framework.
5.  **Update Configuration:** Add the feature flag to the settings schema.
6.  **Verify:** Ensure `AgentEvent` stream fidelity is maintained so the UI
    requires no changes.
