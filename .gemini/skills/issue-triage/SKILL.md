...
## Guidelines
- **Exactly One Label:** You MUST select exactly one `area/` label.
- **Fallback:** If you cannot confidently determine the correct `area/` label 
  from the definitions, you must use `area/unknown`.
- **Pure JSON:** Your final output must contain only valid JSON without 
  explanations or markdown code blocks unless explicitly requested.
